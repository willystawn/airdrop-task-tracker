...
export const TaskItem: React.FC<TaskItemProps> = ({ task, onToggleComplete, globalTagDefinitions }) => {
  ...
  const handleToggle = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (isEndedTask) return; // Prevent toggling for "Ended" tasks
  ...
  const titleContent = (
    <div className="flex items-center w-full gap-3">
      <button
        ...
        className={`p-1 flex-shrink-0 focus:outline-none rounded-full ${isEndedTask ? 'cursor-default' : 'hover:bg-base-300/50'}`}
      ...
      <div className="flex-grow text-left overflow-hidden">
        <h3 className={`font-semibold ${isEffectivelyCompleted ? 'line-through text-base-content-secondary' : 'text-base-content'} group-hover:text-primary transition-colors`}>
